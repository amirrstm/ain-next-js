...
const InterestForm: React.FC = () => {
  useI18nZodErrors('resume.interest')
  const t = useTranslations('Resume.Interest')
  const form = useFormContext<ResumeFormType>()

  const { append, remove, move, fields } = useFieldArray({ control: form.control, name: 'interests' })
  ...
  return (
    <div className="rounded-lg border border-muted bg-card">
      ...
      <div className="space-y-6 p-4">
        ...
        <AddNew onClick={() => append({ keywords: [], name: '' })} title={t('AddNew')} />
      </div>
    </div>
  )
}
