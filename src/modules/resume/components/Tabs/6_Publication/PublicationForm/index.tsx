...
const SinglePublicationFrom: React.FC<Props> = ({ position, fieldId, hasMove, moveCard, remove }) => {
    ...
    >
      ...
      <div className="col-span-12">
        <FormField
          ...
          name={`publications.${position}.name`}
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('resume.publication.name')}</FormLabel>
              <FormControl>
      ...
      <div className="col-span-12 sm:col-span-8">
        <FormField
          control={form.control}
          name={`publications.${position}.publisher`}
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('resume.publication.publisher')}</FormLabel>
              <FormControl>
                <Input {...field} inputSize="sm" />
              </FormControl>
      ...
      <div className="col-span-12 sm:col-span-4">
        <FormField
          control={form.control}
          name={`publications.${position}.releaseDate`}
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('resume.publication.releaseDate')}</FormLabel>
              <FormControl>
                <MonthPicker {...field} />
              </FormControl>
      ...
      <div className="col-span-12">
        <FormField
          control={form.control}
          name={`publications.${position}.summary`}
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t('resume.publication.summary')}</FormLabel>
              <FormControl>
...
export default SinglePublicationFrom
