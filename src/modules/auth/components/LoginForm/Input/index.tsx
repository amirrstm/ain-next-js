...
const Input = React.forwardRef<HTMLInputElement, InputProps>(({ loading, className, type, onRequestSubmit, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="-translate-y-1/2 absolute top-1/2 right-2 flex h-7 w-7 cursor-pointer items-center justify-center rounded-full border border-neutral-400"
        onClick={onRequestSubmit}
      >
        {loading ? <IconProgress className="h-4 w-4 animate-spin" /> : <IconArrowRight className="h-4 w-4 text-neutral-400" />}
      </div>
    </motion.div>
  )
})
