...
const LanguageForm: React.FC = () => {
  useI18nZodErrors('resume.language')
  const t = useTranslations('Resume.Language')
  const form = useFormContext<ResumeFormType>()

  const { append, remove, move, fields } = useFieldArray({ control: form.control, name: 'languages' })
  ...
  return (
    <div className="bg-background rounded-lg border border-muted">
      ...
      <div className="p-4 space-y-6">
        ...
        <AddNew title={t('AddNew')} onClick={() => append(resumeDefaultValues.languages[0])} />
      </div>
    </div>
  )
}
